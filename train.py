...
def train(train_loader, model, criterion, optimizer, epoch):
    ...
    for i, (input, input1, target) in enumerate(train_loader):
        ...
        #print(output1.shape)
        #output2 = torch.cat((output,output1),1)
        #print(output2.shape)
        #cnn = CNN()
        #cnn.cuda()
...
def validate(val_loader, model, criterion, epoch):
    ...
    with torch.no_grad():
        losses = []
        for i, (input, input1, target) in enumerate(val_loader):
            # compute output
            target.requires_grad = False
            target = target.cuda(non_blocking=True)
            #output = model(input)
